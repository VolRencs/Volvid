...
type playlistItem struct{ PlaylistEntry; selected bool }
func (p playlistItem) FilterValue() string { return p.Title }

type playlistDelegate struct{}
func (playlistDelegate) Height() int  { return 1 }
func (playlistDelegate) Spacing() int { return 0 }
func (playlistDelegate) Update(_ tea.Msg, _ *list.Model) tea.Cmd { return nil }
func (d playlistDelegate) Render(w io.Writer, m list.Model, index int, item list.Item) {
	p := item.(playlistItem)
	cur := index == m.Index()
	ar := "  "; if cur { ar = sTitle.Render("> ") }
	chk := sGray.Render("[ ]"); if p.selected { chk = sOk.Render("[✔]") }
	num := sDim.Render(fmt.Sprintf("%4d", p.Index))
	ttl := lipgloss.NewStyle().Width(44).Inline(true).Render(trunc(p.Title, 40))
	if cur { ttl = sNormal.Render(ttl) } else { ttl = sDim.Render(ttl) }
	dur := sDim.Render(FmtDuration(p.Duration))
	fmt.Fprint(w, ar+chk+"  "+num+"  "+ttl+"  "+dur)
}
...
func newModel() model {
	...
	return model{scr: scrUpdateCheck, sp: sp, progDep: pg, urlInput: ti, plInput: pi, numWorkers: 1}
}
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case msgPlaylistFetched:
		if msg.err != nil || msg.info == nil {
			...
			m.menuList = createMenuList([]string{"▲ Лучшее качество (HD·4K)", "▼ Экономичное (360p)", "♪ Только аудио (MP3)"})
			return m, nil
		}
		m.plInfo = msg.info
		items := make([]list.Item, len(msg.info.Entries))
		for i, e := range msg.info.Entries {
			items[i] = playlistItem{PlaylistEntry: e, selected: false}
		}
		m.plList = list.New(items, playlistDelegate{}, 70, 18)
		m.plList.SetShowStatusBar(false); m.plList.SetShowTitle(false); m.plList.SetShowHelp(false)
		m.plList.SetShowPagination(false); m.plList.SetFilteringEnabled(false)
		m.scr = scrPlaylist
		return m, nil
	case msgDlUpdate:
...
func (m model) handleDlUpdate(u DlUpdate) (tea.Model, tea.Cmd) {
	...
	if u.Type == EvDone {
		...
		if m.dlTotal > 0 {
			pct := float64(m.dlDone+m.dlFailed) / float64(m.dlTotal) * 100
			extraCmds = append(extraCmds, m.overallProg.SetPercent(pct/100))
		}
...
func (m model) handleKey(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	busy := m.scr == scrUpdateCheck || m.scr == scrUpdateDl || m.scr == scrDepDl || m.scr == scrDepUpdate || m.scr == scrDownload || m.scr == scrPlaylistFetch
	...
	if m.scr == scrPlaylist {
		if m.plInputMode {
			...
			case "enter":
				...
				for i := range m.plList.Items() {
					item := m.plList.Items()[i].(playlistItem)
					item.selected = false
					for _, idx := range indices { if item.Index == idx { item.selected = true; break } }
					m.plList.SetItem(i, item)
				}
		...
		case "a", "а":
			all := true
			for _, it := range m.plList.Items() { if !it.(playlistItem).selected { all = false; break } }
			for i := range m.plList.Items() {
				item := m.plList.Items()[i].(playlistItem)
				item.selected = !all
				m.plList.SetItem(i, item)
		...
		case "enter":
			sel := make([]PlaylistEntry, 0, len(m.plList.Items()))
			for _, it := range m.plList.Items() { if it.(playlistItem).selected { sel = append(sel, it.(playlistItem).PlaylistEntry) } }
...
func hintLine(s string) string { return "\n" + sDim.Render(s) }
...
func (m model) renderBody() string {
	...
	case scrUpdateReady:
		...
		b.WriteString(hintLine("↑↓ — выбор   Enter — подтвердить"))
	case scrFFmpegAsk:
...
func viewPlaylist(m model) string {
	...
	if m.plInputMode {
		...
		selected := 0
		for _, it := range m.plList.Items() { if it.(playlistItem).selected { selected++ } }
		b.WriteString(sDim.Render("  Выбрано: ") + sOk.Render(fmt.Sprintf("%d", selected)) + sDim.Render(fmt.Sprintf(" / %d", total)) + "\n")
		if m.plInputErr != "" { b.WriteString(sErr.Render("  ✘  "+m.plInputErr) + "\n") }
...
func viewDownload(m model) string {
	...
	if m.dlTotal > 0 {
		...
		b.WriteString("\n  " + sOk.Render(fmt.Sprintf("✔ %d", m.dlDone)) + "  " + sErr.Render(fmt.Sprintf("✘ %d", m.dlFailed)) + "  " + sDim.Render(fmt.Sprintf("◷ %d в очереди", m.dlTotal-done)) + "\n")
...
func viewSlot(i int, s slotState, bar string, badge bool) string {
	...
	ttl := sDim.Render(ttlRaw); if s.done { ttl = sNormal.Render(ttlRaw) }
	row1 := pre + ttl
	switch {
